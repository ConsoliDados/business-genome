...
- **active** — in use.
- **dormant** — not used now but kept for later → its folder is moved to `99-dormant/<module>/`
...
- **external:<tool>** — handled by an outside system (e.g. `crm: external:bitrix24`); see
  `97-config/integrations/<tool>.md` for the sync mapping.

## Languages
Content languages for the model library (proposals/contracts/meetings/emails). Models live
under `96-templates/<kind>/<lang>/`. `en/` ships by default; `init` generates the others you pick.
...
## Pipelines
The commercial/ops pipelines you run — one note each in `97-config/pipelines/`. Ship generic:
...
`97-config/integrations/`. The vault **models** the integration (what/which fields/direction);
