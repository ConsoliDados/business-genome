...
**Writes:** `editorial/ideas/`, `editorial/posts/`

## Steps
1. [[capture-idea]] — structure the idea in `editorial/ideas/`. **If the channel isn't
   stated, ASK** (blog / LinkedIn / Substack / X / Instagram / youtube-script). Never assume.
2. [[draft-post]] — write the channel-formatted draft in `editorial/posts/` (respect the
   channel rules in `editorial/guides/`).
...
## Guard-rails
- Channel unknown → ASK. Keep ConsoliDados voice (see `editorial/guides/voice.md`).
