...
**When:** `_config/genome.md` has `status: template-default`.
**Reads/Writes:** `_config/genome.md`, `_config/pipelines/*`, `_config/integrations/*`,
`_templates/*`, and moves folders into `_dormant/`.
...
- Generate the **model library** in the chosen languages: copy `_templates/<kind>/en/` →
  `<kind>/<lang>/` and adapt/translate. Explain each generic model as you go.
- **Dormant** module → move its folder to `_dormant/<module>/` (out of dashboards; reactivate by
...
> what you'd change in `_config/` and `_templates/`.
