...
**Writes:** `_config/integrations/<tool>.md` (from `_integration.md`) + updates [[genome]].

## Steps
1. Copy `_config/integrations/_integration.md` → `<tool>.md`.
...
3. Set the module to `external:<tool>` in `_config/genome.md` (or `dormant` if not live yet).
