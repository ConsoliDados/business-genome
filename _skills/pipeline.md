...
**Writes:** `_config/pipelines/<name>.md` (+ models in `_templates/`)
...
2. Point each stage at models in `_templates/{meetings,emails,proposals,contracts}/<lang>/` —
   reuse if present, else create generic ones ([[meeting-template]], [[email-template]]).
3. Register the pipeline in `_config/genome.md` (`pipelines:`).
